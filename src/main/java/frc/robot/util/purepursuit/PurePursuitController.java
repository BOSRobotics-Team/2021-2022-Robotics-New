...
public class PurePursuitController {
   ...
   * @param drivebaseWidth Drivebase width in meters
   * @param kMaxOutput Maximum output percent
   * @param followReverse Should the path be followed with the back of the chassis?
   */
  public PurePursuitController(
      Path path,
      double lookaheadDistance,
    ...
    // Configure follower
    m_follower = new PathFollower(path, lookaheadDistance, lookaheadGain, drivebaseWidth);
  ...
  private Translation2d getRotatedGoal(Pose2d robotPose) {
    ...
    if (followReverse) {
      ...
      alpha =
          Math.atan2(chassisCoords.getY() - goal.getY(), chassisCoords.getX() - goal.getX())
              - chassisAngle.getRadians();
    } else {
      alpha =
          Math.atan2(goal.getY() - chassisCoords.getY(), goal.getX() - chassisCoords.getX())
              - chassisAngle.getRadians();
    }
    ...
    if (this.epsilonEquals(alpha, 0.0, 1e-12)) {
      delta =
          MathUtil.clamp(
              Math.hypot(goal.getX() - chassisCoords.getX(), goal.getY() - chassisCoords.getY()),
              -1,
              1);
    } else {
   ...
   *
   * @param robotPose Robot's pose
   * @return Vector facing in the desired direction of travel, with magnitude matching the desired
   *     speed
   */
  public Translation2d calculateHolonomic(Pose2d robotPose) {
    ...
    // Clamp the speeds
    double x =
        MathUtil.clamp(goal.getX() - robotPose.getTranslation().getX(), -kMaxOutput, kMaxOutput);
    double y =
        MathUtil.clamp(goal.getY() - robotPose.getTranslation().getY(), -kMaxOutput, kMaxOutput);
