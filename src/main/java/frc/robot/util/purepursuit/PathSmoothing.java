...
public class PathSmoothing {
   ...
   */
  public static ArrayList<Translation2d> smooth(
    ...
    while (change >= tolerance && countdown > 0) {
      ...
      for (int i = 1; i < positions.size() - 1; i++) {
        ...
        Translation2d firstFactor =
            previousPointState.plus(positions.get(i).minus(previousPointState).times(weight_data));

        // Compute the second erosion factor
        Translation2d secondFactor =
            newPositions
                .get(i - 1)
                .plus(newPositions.get(i + 1))
                .minus(previousPointState.times(2.0))
                .times(weight_smooth);
