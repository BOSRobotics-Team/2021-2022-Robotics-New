...
/**
 * A lookahead finder for finding new points in a 2D path. <br>
 * <br>
...
public class PathFollower {
   ...
   */
  public PathFollower(
      Path path, double lookaheadDistance, double lookaheadGain, double drivebaseWidth) {
    this.m_path = path;
  ...
  public Translation2d getNextPoint(Pose2d robotPose) {
    ...
    if (m_lastLookaheadIndex == null) {
      ...
      for (int i = 0; i < m_path.getPoses().length; i++) {
        ...
        if (d
            < Math.hypot(
                robotVector.getX() - nearest.getX(), robotVector.getY() - nearest.getY())) {
      ...
      while (thisDist <= m_lookaheadGain) {  
        ...
        if (thisDist < nextDist || nextIndex == lastIndex) {
