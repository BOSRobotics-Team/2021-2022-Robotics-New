...
/**
 * A "Path" is a list of closely spaces points in space for a robot to follow. The default Path
 * class will generate paths with any number of waypoints, and create linear sub-paths.
 */
public class Path {
  ...
  protected void saveAndLogGenerationTime() {
    ...
    // RobotLogger.getInstance().log(String.format("%s generation finished in: %.4fms", name,
    // pathGenTimeMs));
  }
  ...
  /**
   * Get a chart showing every generated path point in 2D space. Can be written to disk for
   * debugging and demos.
   *
  ...
  public XYChart getPathVisualization() {
    ...
    // Build as a chart
    // XYChart chart = QuickChart.getChart("Generated Path", "X (meters)", "Y (meters)", "path",
    // xData, yData);
    XYChart chart =
        new XYChartBuilder()
            .width(1000)
