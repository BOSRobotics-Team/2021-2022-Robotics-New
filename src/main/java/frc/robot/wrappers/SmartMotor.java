...
public class SmartMotor extends WPI_TalonFX {
  ...
  private final Convertor convertor = new Convertor(kSensorUnitsPerRotation);
  ...
  public SmartMotor(int deviceNumber) {
    ...
    this.configSelectedFeedbackSensor(
        TalonFXFeedbackDevice.IntegratedSensor, PID_PRIMARY, kTimeoutMs);
     ...
     * when driving Talon Forward / Requesting Postive Output Phase sensor to have positive
     * increment when driving Talon Forward (Green LED) Talon FX does not need sensor phase set for
     * its integrated sensor This is because it will always be correct if the selected feedback
     * device is integrated sensor (default value) and the user calls getSelectedSensor* to get the
     * sensor's position/velocity.
     */
  ...
  public void setDistanceConfigs(Gains gains) {
    ...
    talonConfig.primaryPID.selectedFeedbackSensor =
        TalonFXFeedbackDevice.IntegratedSensor.toFeedbackDevice(); // Local Feedback Source
    talonConfig.neutralDeadband = kNeutralDeadband;

    /**
     * Max out the peak output (for all modes). However you can limit the output of a given PID
     * object with configClosedLoopPeakOutput().
     */
    ...
    if (this.getInverted()) {
      ...
      talonConfig.sum0Term =
          TalonFXFeedbackDevice.RemoteSensor0.toFeedbackDevice(); // Aux Selected Sensor
      talonConfig.sum1Term =
  ...
  public void setDistanceAndTurnConfigs(int sensorID, Gains dgains, Gains gains) {
    ...
    talonConfig.remoteFilter0.remoteSensorSource =
    ...
    if (this.getInverted()) {
      ...
      talonConfig.auxiliaryPID.selectedFeedbackSensor =
          TalonFXFeedbackDevice.SensorSum.toFeedbackDevice(); // Sum0 + Sum1
  ...
  public void setTarget(double meters, double aux) {
    this.set(
        ...
        convertor.distanceMetersToNativeUnits(meters),
        DemandType.AuxPID,
        aux);
  }
  ...
  public void logPeriodic() {
    ...
    //      SmartDashboard.putNumber(name + "ClosedLoopError",
    // this.getClosedLoopError(PID_PRIMARY));

    /* Print the Active Trajectory Point Motion Magic is servoing towards */
    //        SmartDashboard.putNumber(name + "ClosedLoopTarget",
    // this.getClosedLoopTarget(PID_PRIMARY));
    //        SmartDashboard.putNumber(name + "ActTrajVelocity",
    // this.getActiveTrajectoryVelocity());
    //        SmartDashboard.putNumber(name + "ActTrajPosition",
    // this.getActiveTrajectoryPosition());
  }
